...
NextEvent(MovingParticle* p, vector<MovingParticle*>& particles)
{
	...
	{
		CParticleF p2 = p->move(-time);
		CParticleF q2 = p->next->move(-time);
		pair<float,float> param = _IntersectConvexPolygon::intersect(p->p0, p2, p->next->p0, q2);
		float t0 = param.first; //Max(param.first, param.second);
		if(t0>mint)
		{
			float t = t0;
			ev = EventStruct(t, CollisionEvent, p, p->next);
		}
	}
	{
		CParticleF ol = p->move(-time);
		CParticleF ul(p->v[0], p->v[1], 1.0);
		for(int j=0; j<particles.size(); ++j)
		{
			if(p==particles[j] || p->prev==particles[j]) continue;
			if(particles[j]->id==1556)
				j+=0;
			CParticleF op = particles[j]->move(-time);
			CParticleF oq = particles[j]->next->move(-time);
			...
			if(t0 > mint) 
			{
				...
				if(t < ev.t)
				{
					...
					CParticleF op2 = particles[j]->move(t0+1.0f-time);
					CParticleF oq2 = particles[j]->next->move(t0+1.0f-time);
...
{
	int poid = 73; //836; //122; //135;
	int paid = 1743; //745; //1437;
