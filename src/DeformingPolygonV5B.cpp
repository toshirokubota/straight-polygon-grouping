...
DeformingPolygon::deform(float period)
{
	...
	for(int i=0; i<particles.size(); ++i)
	{
		...
		{
			float etime = particles[i]->event.t;
			EventType type = particles[i]->event.type;
			EventStruct ev = NextEvent(particles[i], particles);
			particles[i]->event = ev;
			if(type != UnknownEvent && ev.t < etime)
			{
				//particles[i]->event.t = etime; //event time cannot be smaller? Is it?
			}
		}
