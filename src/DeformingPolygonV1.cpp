...
quickFinish(vector<MovingParticle*> particles)
{
	...
	if(particles.size()==2)
	{
		...
		CParticleF q(x1, y1);
		MovingParticle* pa = factory->makeParticle(q, Axis);
		for(int i=0; i<2; ++i)
		{
			particles[i]->p = q;
	...
	while(particles.size()>1) //at least two points are needed. else particles can vanish.
	{
		...
		for(int i=0; i<particles.size(); ++i)
		{
			...
			else if(particles.size()>=3)
			{
				...
				{
					MovingParticle* z = factory->makeParticle(p->p, Collide); //Axis);;
			 ...
			 map<MovingParticle*,MovingParticle*>& cmap)
{
	...
	//check for each polygon if it is a merged segment (area of 0).
	//if so, then create axis particles to indicate the presence of axis.
	/*float eps = 0.001;
	for(int i=polygons.size()-1; i>=0; --i)
	{
		//if(polygons[i].size()==4) //possibly a merged straight line
		{
			vector<CParticleF> vp;
			float maxx=-std::numeric_limits<float>::infinity(), minx=std::numeric_limits<float>::infinity();
			float maxy=-std::numeric_limits<float>::infinity(), miny=std::numeric_limits<float>::infinity();
			for(int j=0; j<polygons[i].size(); ++j)
			{
				CParticleF p = polygons[i][j]->p;
				vp.push_back(p);
				maxx = Max(maxx, p.m_X); minx = Min(minx, p.m_X);
				maxy = Max(maxy, p.m_Y); miny = Min(miny, p.m_Y);
			}
			...
			if(area < eps)
			{
				...
				if(maxx-minx > maxy-miny)
				{
					for(int j=0; j<polygons[i].size(); ++j)
					{
						pairs.push_back(pair<float,MovingParticle*>(polygons[i][j]->p.m_X, polygons[i][j]));
					}
				}
				else
				{
					for(int j=0; j<polygons[i].size(); ++j)
					{
						pairs.push_back(pair<float,MovingParticle*>(polygons[i][j]->p.m_Y, polygons[i][j]));
					}
				}
				sort(pairs.begin(), pairs.end());
				MovingParticle* p1 = factory->makeParticle(pairs[0].second->p, Axis);
				setRelation(pairs[0].second, p1);
				MovingParticle* p2 = factory->makeParticle(pairs[pairs.size()-1].second->p, Axis);
				setRelation(pairs[pairs.size()-1].second, p2);
				CParticleF mid((p1->p.m_X+p2->p.m_X)/2.0, (p1->p.m_Y+p2->p.m_Y)/2.0);
				p1->p = mid;
				p2->p = mid;
		...
		for(int j=0; j<N; ++j)
		{
			...
			else
			{
				...
				if(overlap(p->p, q->p, q->next->p, p->p0, y, eps))//needs a dummy...
				{
					...
					//now figure out where x was on the initial segment between q->p0 and q->next->p0.
					//MovingParticle* x0 = factory->makeParticle(y, Dummy);					
					/*if(Abs(q->p0.m_X-q->next->p0.m_X) > Abs(q->p0.m_Y-q->next->p0.m_Y))
					{
						float t = (x->p.m_X - q->p.m_X) / (q->next->p.m_X - q->p.m_X);
						x->p0.m_X = (1-t)*q->p0.m_X + t*q->next->p0.m_X;
						x->p0.m_Y = (1-t)*q->p0.m_Y + t*q->next->p0.m_Y;
					}
					else
					{
						float t = (x->p.m_Y - q->p.m_Y) / (q->next->p.m_Y - q->p.m_Y);
						x->p0.m_X = (1-t)*q->p0.m_X + t*q->next->p0.m_X;
						x->p0.m_Y = (1-t)*q->p0.m_Y + t*q->next->p0.m_Y;
					}*/
					//setRelation(x0, x);
