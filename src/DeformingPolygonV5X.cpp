...
{
	pair<double,double> s(std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN());
	if(a == 0 && b==0)
	{
	}
	else if(a == 0 && b!=0)
	...
	else //a != 0
	{
		...
		{
			s.first = (-b - sqrt(det))/(4*a*c);
			s.second = (-b + sqrt(det))/(4*a*c);
		}
	}
	//s.first = c / b;
	if(s.second < s.first)
...
NextEvent(MovingParticle* p, vector<MovingParticle*>& particles)
{
	...
	CParticleF p1=p->p;
	float u1[2] = {p->v[0], p->v[1]};
	for(int i=0; i<particles.size(); ++i)
	{
		MovingParticle* q = particles[i];
		if(p==q || p->prev==q) continue;
		CParticleF p2=q->p;
		float u2[2] = {q->v[0], q->v[1]};
		CParticleF p3=q->next->p;
		float u3[2] = {q->next->v[0], q->next->v[1]};

		float a1=p1.m_X - p3.m_X;
		float a2=p2.m_X - p3.m_X;
		float b1=p1.m_Y - p3.m_Y;
		float b2=p2.m_Y - p3.m_Y;
		float c1=u1[0]-u3[0];
		float c2=u2[0]-u3[0];
		float d1=u1[1]-u3[1];
		float d2=u2[1]-u3[1];
		float prod = a2*d2-b2*c2;
		assert(Abs(prod) < 1.0e-5);
		pair<double,double> solution = quadraticEquation(c2*d1-c1*d2, b1*c2+a2*d1-a1*d2-b2*c1, a2*b1-a1*b2);
		...
		//float s = (a1 + t*c1) / (a2 + t*c2); //linear interpolation parameter
		float s = (b1 + t*d1) / (b2 + t*d2); //linear interpolation parameter
		if(s >= 0 && s < 1.0) //the intersection has to be between end points.
		{
			CParticleF r1(p1.m_X+t*u1[0], p1.m_Y+t*u1[1]);
			CParticleF r2(p2.m_X+t*u2[0], p2.m_Y+t*u2[1]);
			CParticleF r3(p3.m_X+t*u3[0], p3.m_Y+t*u3[1]);
			CParticleF r4(r2.m_X*s + r3.m_X*(1-s), r2.m_Y*s + r3.m_Y*(1-s));
			if(t < ev.t)
			{
				if(q==p->next) //collision
				{
					ev = EventStruct(t, CollisionEvent, p, q);
				}
				else
				{
					ev = EventStruct(t, SplitEvent, p, q);
				}
			}
		}
	}
...
NextEvent(MovingParticle* p, vector<MovingParticle*>& particles)
{
	...
	EventStruct ev(std::numeric_limits<float>::infinity(), UnknownEvent, p);
	{
		...
		if(t0>=0)
		{
			ev = EventStruct(t0 + ptime, CollisionEvent, p, p->next);
		}
	}
	{
		CParticleF ol = p->p;
		//CParticleF ol(p->p0.m_X, p->p0.m_Y, -ptime);
		CParticleF ul(p->v[0], p->v[1], 1.0);
		for(int j=0; j<particles.size(); ++j)
		{
			if(p==particles[j] || p->prev==particles[j]) continue;
			if(particles[j]->id==1425 || particles[j]->id==1426)
				j+=0;
			...
			if(t0 >= 0) 
			{
				...
				if(t < ev.t)
				{
					...
					CParticleF op2 = particles[j]->move(z+1.0f);
					CParticleF oq2 = particles[j]->next->move(z+1.0f);
