...
bool
SingleComponent4(int values[3][3], int value)
{
	...
	if(count == 0) return false; //has to have at least one component
	Node<int>* rep = NULL;
	int ioff[]={0, 1, 1, 2};
	int joff[]={1, 0, 2, 1};
	for(int k=0; k<4; ++k)
...
bool 
SingleComponent8(int values[3][3], int value)
{
	static Node<int>* nodes[3][3]={{0, 0, 0}, {0, 0, 0}, {0, 0, 0}};
	if(nodes[0][0]==NULL)
	{
		 ...
		 const int* dims)
{
	...
	vals[1][1]=2;
	bool b1 = SingleComponent8(vals, 1);
	bool b2 = SingleComponent4(vals, 0);
	return b1 && b2;
	//return SingleComponent8(vals, 1) && SingleComponent4(vals, 0);
}
