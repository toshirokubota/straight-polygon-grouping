...
MovingParticle*
applyCollision(MovingParticle* p)
{
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	while (time < 10.0f)
	{
		...
		{
			EventStruct ev(numeric_limits<float>::infinity(), UnknownEvent, Q[i]);
			if (checkForCollision(Q[i]))
			{
			}
			else if (checkForSplit(Q[i]))
			{
			}
			//check for collision.
			//else check for split
		}
	}
	if(nlhs >= 1)
	{
